...
namespace MH.WaterCausticsModules.TexGen {
    ...
    public class WaterCausticsTexGeneratorEditor : Editor {
        ...
        private SerializedProperty m_unscaledTime;
        ...
        public override void OnInspectorGUI () {
            ...
            if (EditorUserBuildSettings.activeBuildTarget == BuildTarget.WebGL) {
                ...
                labelWarning ($"The current build target settings is WebGL. \nWaterCausticsTexGenerator is not compatible with WebGL, because WebGL does not support Compute Shader. \nIt works in the editor, but not when building for WebGL.");
                EditorGUILayout.Space (10);
            }
#endif
                ...
                if (expandSubGroup (m_pause, true, "Wave")) {
                    ...
                    using (new DisableScope (!m_pause.boolValue)) {
                        ...
                        using (new IndentScope (-2f, 0f)) {
                            EditorGUILayout.PropertyField (m_unscaledTime, new GUIContent ("Unscaled Time", "Use unscaled time."));
                        }
